...
from utils.datasets.augmentations.cifar_augmentation import \
...
def get_80MTinyImages(batch_size=100, augm_type='default', shuffle=True,
                      cutout_window=16, num_workers=1, size=32,
    ...
    augm_config = {}
    transform = get_cifar10_augmentation(augm_type,
    ...
    loader = DataLoader(dataset_out, batch_size=batch_size,
...
def _load_cifar_exclusion_idcs(exclude_cifar, exclude_cifar10_1):
    ...
    if exclude_cifar:
        with open(os.path.join(main_idcs_dir, '80mn_cifar_idxs.txt'),
                  'r') as idxs:
            for idx in idxs:
    ...
    if exclude_cifar10_1:
        with open(os.path.join(main_idcs_dir, '80mn_cifar101_idxs.txt'),
                  'r') as idxs:
            for idx in idxs:
                cifar_idxs.append(int(idx))
...
class TinyImagesDataset(Dataset):
    def __init__(self, transform_base, exclude_cifar=False,
                 exclude_cifar10_1=False):
        self.data_location = get_tiny_images_files(False)
        self.memap = np.memmap(self.data_location, mode='r', dtype='uint8',
                               order='C').reshape(TINY_LENGTH, -1)
        ...
        exclusion_idcs = _load_cifar_exclusion_idcs(exclude_cifar,
                                                    exclude_cifar10_1)

        self.included_indices = torch.ones(TINY_LENGTH, dtype=torch.long)
        self.included_indices[exclusion_idcs] = 0
        self.included_indices = torch.nonzero(self.included_indices,
                                              as_tuple=False).squeeze()
        self.length = len(self.included_indices)
        print(f'80M Tiny Images - Length {self.length} -',
              f'Excluding {len(exclusion_idcs)} images')
